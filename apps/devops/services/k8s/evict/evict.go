	...
	*/
	...
	for _, i := range pods.Items {
		if i.Namespace == systemNamespace {
			...
			global.Log.Info(fmt.Sprintf("开始驱逐Node: %v, 节点Namespace: %v下的Pod: %v", nodeName, i.Namespace, i.Name))
			err := EvictsPod(client, i.Name, i.Namespace)
			if err != nil {
				global.Log.Error(fmt.Sprintf("驱逐Pod：%v失败", i.Name))
			}
		}
	}
	global.Log.Info(fmt.Sprintf("已成功从节点: %v 中驱逐所有pod", nodeName))
	return nil
}
